...
// AES 加密
Java_com_lyy_security_NativeCaller_aesEncrypt(JNIEnv *env, jobject thiz, jbyteArray src_) {
    ...
    // LOGD("AES->从 jni 释放数据指针");
    env->ReleaseByteArrayElements(src_, src, 0);

    jbyteArray cipher = env->NewByteArray(cipherText_len);
    // LOGD("AES->在堆中分配 ByteArray 数组对象成功，将拷贝数据到数组中");
...
// AES 解密
extern "C" JNIEXPORT jbyteArray
Java_com_lyy_security_NativeCaller_aesDecrypt(JNIEnv *env, jobject thiz, jbyteArray src_) {
    if (IS_SIGNED_FLAG == FALSE) {
        LOGE("ERROR : Signature error or tampering!");
        throw "Signature error or tampering!";
    }
...
// RSA 私钥加密
extern "C" JNIEXPORT jbyteArray
Java_com_lyy_security_NativeCaller_rsaEncryptPrivate(JNIEnv *env, jobject thiz, jbyteArray src_) {
    // LOGD("RSA->非对称密码算法");
    if (IS_SIGNED_FLAG == FALSE) {
        LOGE("ERROR : Signature error or tampering!");
        throw "Signature error or tampering!";
    ...
    // LOGD("RSA->从字符串读取 RSA 私钥");
    keybio = BIO_new_mem_buf(RSA_PRIVATION_KEY, -1);
    // LOGD("RSA->从 BIO 结构中得到 RSA 结构");
    ...
    // RSA_PKCS1_PADDING 最大加密长度：128-11；RSA_NO_PADDING 最大加密长度：128
    ...
    // LOGD("RSA->从 jni 释放数据指针");
    env->ReleaseByteArrayElements(src_, src, 0);

    jbyteArray cipher = env->NewByteArray(cipherText_offset);
    // LOGD("RSA->在堆中分配 ByteArray 数组对象成功，将拷贝数据到数组中");
...
// RSA 公钥加密
extern "C" JNIEXPORT jbyteArray
Java_com_lyy_security_NativeCaller_rsaEncryptPublic(JNIEnv *env, jobject thiz, jbyteArray src_) {
    // LOGD("RSA->非对称密码算法");
    if (IS_SIGNED_FLAG == FALSE) {
        LOGE("ERROR : Signature error or tampering!");
        throw "Signature error or tampering!";
...
// RSA 私钥解密
extern "C" JNIEXPORT jbyteArray
Java_com_lyy_security_NativeCaller_rsaDecryptPrivate(JNIEnv *env, jobject thiz, jbyteArray src_) {
    // LOGD("RSA->非对称密码算法");
    if (IS_SIGNED_FLAG == FALSE) {
        LOGE("ERROR : Signature error or tampering!");
        throw "Signature error or tampering!";
...
// RSA 公钥解密
extern "C" JNIEXPORT jbyteArray
Java_com_lyy_security_NativeCaller_rsaDecryptPublic(JNIEnv *env, jobject thiz, jbyteArray src_) {
    // LOGD("RSA->非对称密码算法");
    if (IS_SIGNED_FLAG == FALSE) {
        LOGE("ERROR : Signature error or tampering!");
        throw "Signature error or tampering!";
...
// RSA 私钥签名
extern "C" JNIEXPORT jbyteArray
Java_com_lyy_security_NativeCaller_rsaSignPrivate(JNIEnv *env, jobject thiz, jbyteArray src_) {
    // LOGD("RSA->非对称密码算法");
    jbyte *src = env->GetByteArrayElements(src_, NULL);
    jsize src_Len = env->GetArrayLength(src_);
...
Java_com_lyy_security_NativeCaller_rsaVerifyPublic(JNIEnv *env, jobject thiz, jbyteArray src_, jbyteArray sign_) {
    // LOGD("RSA->非对称密码算法");
    jbyte *src = env->GetByteArrayElements(src_, NULL);
...
// 用 apk 签名的 hash1 进行验证，从 apk 获取的 hash1 的值与我们配置的常量 hash1 是否一致
extern "C" JNIEXPORT jboolean JNICALL
Java_com_lyy_security_NativeCaller_verifyApkSignHash1(JNIEnv *env, jclass clazz, jobject m_context) {
